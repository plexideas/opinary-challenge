...
import './styles.css';
import { CONFIG_DATA_ATTRIBUTE, WIDGET_CLASSNAME } from './configs';

const widgetContainers = document.querySelectorAll(WIDGET_CLASSNAME);
const widgetConfigList = []; // contains all loaded widget configs in the app / page

widgetContainers.forEach(container => {
    const widget = createRoot(container);
    const config = container.getAttribute(CONFIG_DATA_ATTRIBUTE);
